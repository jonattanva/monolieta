...
import Trash from '../icon/trash/index.jsx'
...
const Classes = memo((props) => {
    ...
    const onClassAdded = useCallback(() => {
        if (!props.onClassAdded) {
            return
        }
    ...
    const onClassRemoved = () => {
        if (props.onClassRemoved) {
            const selected = dataSource.find((value) => value.checked)
            if (selected) {
                props.onClassRemoved(dataSource)
            }
        }
    }

    const onSelectHandle = useCallback((id, checked) => {
    ...
    const onChangeNameHandle = useCallback((id, name) => {
        setDataSource((current) => {
            return current.map((value) => {
                if (value.id === id) {
                    value.name = name
    ...
    const onChangeColorHandle = useCallback((id, color) => {
        setDataSource((current) => {
            return current.map((value) => {
                if (value.id === id) {
                    value.color = color
    ...
    return (
        <Fragment>
            <Header>
                <Search />
                <Action>
                    <Wrapper onClick={ onClassAdded } role="button">
                        <Plus width="20" height="20" />
                        <div>Add class</div>
                    </Wrapper>
                    <Separator />
                    <Item>
                        <Button title="Delete" onClick={ onClassRemoved } role="button">
...
Classes.propTypes = {
    ...
    /** It is called every time a new class is added */
    onClassAdded: PropTypes.func,

    /** It is called every time a class is removed */
    onClassRemoved: PropTypes.func
}
